 ...
 * ========================================================================== */
import { panel, kvList, table, badge } from "../components.js";
...
function itemsTable(items) {
	...
	const rows = list.slice(0, 40).map((it) => ({
		...
		cells: [
			...
			formatGp(num(it.gePriceEach) ?? num(it.gePrice)),
			formatGp(num(it.haPriceEach) ?? num(it.haPrice)),
...
export function renderInventory(status) {
	...
	el.innerHTML = `
		<div class="grid cols-4" style="margin-bottom:var(--gap)">
			${panel({ title: "Carried (GE)", iconName: "coins", body: `<div class="big-stat">${formatGp(num(wealth.totalCarriedValueGe))}</div>` })}
			${panel({ title: "Carried (HA)", iconName: "wand-2", body: `<div class="big-stat">${formatGp(num(wealth.totalCarriedValueHa))}</div>` })}
			${panel({ title: "Equipment (GE)", iconName: "shield", body: `<div class="big-stat">${formatGp(num(equip.gePrice) ?? num(wealth.equipmentGe))}</div>` })}
			${panel({ title: "Risked (GE)", iconName: "alert-triangle", body: `<div class="big-stat">${formatGp(num(wealth.riskedValueGeApprox))}</div>` })}
		</div>
