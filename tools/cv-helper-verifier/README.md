...
- The synced config panel loads `/automation/mob-farmer/config`, saves validated changes back to RuneLite, supports readable JSON import/export, and can edit action slot hotkey text such as `F12`, `CTRL+1`, `ALT+Q`, or `NOT_SET`.
- Mob-farmer profiles are saved locally in the browser and can be saved, loaded into the form, duplicated, deleted, imported, and exported before applying to RuneLite.
...
- Mining and woodcutting cards expose their own dry/live controls, presets, synced config, import/export JSON, reachable target tables, path distance, inventory GP/HA value, and drop-candidate diagnostics.
...
- Use `Show raw JSON` when you need the exact mob-farmer payload, and the compact log shows the last few step/start/stop actions sent from the verifier.
