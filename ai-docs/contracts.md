...
- Farmer loot/survival flow should remain ordered and inspectable: HP/food guard first, optional intermediate inventory actions, optional loot during combat, configurable attack-before-loot while idle, then loot fallback when no valid attack target exists. Current inventory handling reports free slots, protected never-drop items, and the lowest-value unprotected drop candidate, but automatic dropping is intentionally deferred.
