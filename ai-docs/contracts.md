...
## 2026-06-28 UI, Actions, and Inventory Additions

- WebHelper Actions uses the same 22-slot config payload as the RuneLite sidebar. `POST /automation/actions/run?slot=N` queues the existing executor and `POST /automation/actions/reset-sequence?slot=N` clears only that slot's sequence pointer. Action-slot payloads expose `sequenceIndex` and a redacted `lastRun` result. Native config intentionally exposes the stable global subset and slots 1-4; sidebar/WebHelper are the full slots 1-22 editors.
- OSR-19 wires Mob Farmer into `InventoryDropService` rather than a mob-only drop primitive. A replacement drop is considered only with a full inventory, a non-empty explicit Mob Farmer drop allowlist, and eligible loot blocked only by inventory capacity. Protected/built-in-safe items always win; the cheapest safe candidate must be below the configured max total GE, and unlisted loot must be worth more than the dropped stack. Explicit always-loot/Ground Items candidates may be treated as required. `/automation/mob-farmer/status.dropPolicy` reports candidates, protected skips, selected drop, replacement loot, value delta, intended/actual action, and failure reason before a live `Drop` menu action is queued.
...
- OSR-14/OSR-50 provide bounded local transition routing. Base collision remains authoritative, but the BFS may return up to four ordered, contingent transition steps when each object is explicitly allowlisted, not denylisted, its exact action is one of `Open`, `Close`, `Enter`, `Pass`, or `Climb-over`, and the matching transition flag is enabled. Results expose `transitionRoute`, `transitionCount`, `routeDepth`, `failedTransitionIndex`, the first pending `doorTransition`, and manual-action diagnostics for unknown or ambiguous objects. Each route step includes index, source/from tile, destination/to tile, object id/name/tile, exact action, available actions, and remaining transition count. Live execution performs only the current first step, re-plans from real collision on the next tick, spaces attempts by 1.2 seconds, and holds after five failed attempts instead of attacking through a blocker. `/pathing/grid?radius=N` accepts 1-100 tiles and visually distinguishes direct reachability, one transition, multi-transition routes, manual-action blockers, collision blockers, and failed steps.
...
- CV Helper action hotkeys use one diagnostic guard shared by the pre-dispatcher and normal `KeyManager` fallback. Non-panic hotkeys are suppressed for the first chat character, active chat/message-layer input, Swing text focus, and an unfocused RuneLite window. `/status.hotkeyGuard` exposes only booleans/lengths, event path, matched key, decision, and reason; it never exports typed text. Panic stop is the documented global exception.
