...
- `POST /login/click` or `GET /login/click` queues a guarded click on RuneLite's visible click-to-play/login widget when the client is on a login screen. If that widget is not visible, it must report/skip rather than clicking arbitrary screen coordinates.
...
- `OSR-10` owns the first mob-farmer automation. Current CV Helper exposes `/automation/mob-farmer/status`, `/automation/mob-farmer/step`, `/automation/mob-farmer/start`, `/automation/mob-farmer/stop`, and `/automation/panic-stop`. The farmer accepts a `target` query parameter with partial name, `id:<npc id>`, or target-list semantics and a `live=true|false` query parameter. It must fail closed unless logged in, local player exists, current combat state is safe according to the configured aggro response, a matching NPC passes validity filters, the selected NPC has a click point, and no other CV action is running. Validity filters include attackable/interactible NPC action metadata, dead/zero-HP filtering, already-engaged target handling, single-vs-multi combat policy, optional line-of-sight, and optional max distance. `/automation/mob-farmer/status` must expose the latest combat decision, candidate skip reasons, `survivalDecision`, `lootDecision`, `inventory`, and `lootCandidates`. Dry mode reports the target/click point without clicking and prints it to in-game chat; live mode performs guarded clicks. Panic stop stops CV loops, invalidates queued loop steps, interrupts the loop sleeper, and clears the action-in-progress guard. Auto-eat and first-pass loot filters are implemented: auto-eat opens inventory when needed, clicks matching food below the HP threshold, and can stop on no food; loot scans visible ground items using allowlist/min-value/blacklist/ownership/radius/inventory-capacity rules. Highlighted-drop integration, real route pathing, menu-option/right-click pickup, login recovery, automatic inventory dropping, and external plan composition are follow-up contracts.
- Future click reliability work should add a menu-option/right-click path so entity targeting can choose `Attack <npc>` even when the left-click point is obstructed by drops, players, or other scene/UI overlap.
- Farmer loot/survival flow should remain ordered and inspectable: HP/food guard first, optional loot during combat, configurable attack-before-loot while idle, then loot fallback when no valid attack target exists. Current inventory handling reports free slots, protected never-drop items, and the lowest-value unprotected drop candidate, but automatic dropping is intentionally deferred.
