...
- `OSR-10` owns the first mob-farmer automation. Current CV Helper exposes `/automation/mob-farmer/status`, `/automation/mob-farmer/step`, `/automation/mob-farmer/start`, and `/automation/mob-farmer/stop`. The farmer accepts a `target` query parameter with partial name or `id:<npc id>` semantics and a `live=true|false` query parameter. It must fail closed unless logged in, not already interacting, a matching NPC entity has a click point, and no other CV action is running. Dry mode reports the target/click point without clicking; live mode performs a guarded click. Auto-eat, loot filters, highlighted-drop integration, pathing, and external plan composition are follow-up contracts.
