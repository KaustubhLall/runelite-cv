...
- The server should prefer a fixed/default local development port. If binding fails, it may fall back to an ephemeral port, but `/status` and the RuneLite panel must expose the active port. The local dev launcher passes `-Dcvhelper.forceLocalExport=true` so stale profile settings cannot disable the verifier/API during development; normal RuneLite launches still honor the `Enable localhost export` config toggle.
...
- OSR-14 Part E adds one-hop local door transitions to the shared pathing kernel. Closed doors remain blocked in base collision; a contingent route is returned only when the door id/name is explicitly present in `mobFarmerDoorAllowlist`, absent from `mobFarmerDoorDenylist`, and the matching auto-open/auto-close flag is enabled. Unknown, unallowlisted, denylisted, or disabled transitions remain unreachable and expose `blockedByDoor`, `manualActionRequired`, `manualActionReason`, and `blockingDoor`; candidate diagnostics also expose `playerWorldLocation`. `blockingDoor`/`doorTransition` include id, name, world/from/to tiles, required action, allowlist status, and available actions. `/pathing/grid` distinguishes `reachable`, `reachableViaDoor`, `blocked-by-door:<id>`, `collision-blocked`, and scene/no-route states. Live mob-farmer execution intercepts a selected contingent route, invokes the exact `Open`/`Close` object menu action, waits for native collision/state confirmation, spaces attempts by 1.2 seconds, caps a transition at five attempts, and holds after timeout instead of resuming clicks or attacking through the blocker. Multi-door routing and non-binary actions such as `Enter`, `Pass`, and `Climb-over` are outside this contract.
- Mob-farmer status exposes configurable recovery loop delay, autorun, startup focus-click, and after-loot combat policy. `/automation/mob-farmer/status.autorun` reports enabled/min-energy/current energy/run-enabled/last result. `/automation/mob-farmer/status.startupFocus` reports whether a guarded canvas-center focus click is enabled, needed, or was attempted. `afterLootCombatMode` can hold targeting after a pickup when an NPC is already tagging the player, or stop the farmer instead of selecting another mob.
...
- `OSR-10` owns the first mob-farmer automation. Current CV Helper exposes `/automation/mob-farmer/status`, `/automation/mob-farmer/config`, `/automation/mob-farmer/focus-click`, `/automation/mob-farmer/step`, `/automation/mob-farmer/start`, `/automation/mob-farmer/stop`, and `/automation/panic-stop`. `GET /automation/mob-farmer/config` returns `{version, generatedAt, settings, schema, actionSlots}` for WebHelper/native config sync. `POST /automation/mob-farmer/config` accepts the same readable JSON shape, validates the whole payload first, returns `400` with `errors` without applying anything on failure, and returns canonical config on success. Action hotkeys can be configured through this endpoint using text such as `F12`, `CTRL+1`, `ALT+Q`, or `NOT_SET`. WebHelper also supports local named mob-farmer profiles saved in browser storage; profiles are versioned JSON payloads and can be saved, loaded, duplicated, deleted, imported, and exported without mutating RuneLite until the user applies the payload to the client. `POST /automation/mob-farmer/focus-click` queues the same guarded startup focus click used by the loop. The farmer accepts a `target` query parameter with partial name, `id:<npc id>`, or target-list semantics and a `live=true|false` query parameter. It must fail closed unless logged in, local player exists, current combat state is safe according to the configured aggro response, a matching NPC passes validity filters, and no other CV action is running. Validity filters include attackable/interactible NPC action metadata, dead/zero-HP filtering, already-engaged target handling, single-vs-multi combat policy, optional line-of-sight, and optional max distance. When attack interaction is `DIRECT_CLICK`, the selected NPC must have a click point; when attack interaction is `MENU_ACTION`, the selected NPC must expose an `Attack` action index and CV Helper invokes the corresponding RuneLite NPC menu action. `/automation/mob-farmer/status` must expose the latest combat decision, candidate skip reasons, target candidates, `survivalDecision`, `intermediateDecision`, `lootDecision`, `lastActionAttempt`, `recentMenuEntries`, `inventory`, and `lootCandidates`. Dry mode reports the target/click/menu action without clicking and prints it to in-game chat; live mode performs guarded interactions. Panic stop stops CV loops, invalidates queued loop steps, interrupts the loop sleeper, and clears the action-in-progress guard. Auto-eat and stack-aware loot filters are implemented: auto-eat opens inventory when needed, invokes matching inventory menu actions (`Eat`, `Drink`, or `Consume`) below the HP threshold, and either stops or warns/continues on no food depending on `Stop if no food`; loot scans visible ground items using allowlist/min-value/blacklist/ownership/radius/inventory-capacity rules plus single-item GE, total stack GE, stack quantity, always-loot-above-stack-value, never-loot-below-stack-value, single HA, and total stack HA diagnostics. Loot candidate reports include item id/name, quantity, GE each/stack, HA each/stack, allowlist/denylist match, Ground Items classification, high-priority state, final decision, and rejection reasons. Loot interaction defaults to RuneLite `Take` menu actions using scene coordinates so hidden/deprioritized Ground Items do not rely on left-click visibility. Before a live `Take`, CV Helper must revalidate that the target ground item still exists on the same scene tile and remains selectable; stale or rejected loot attempts return control to the state machine instead of trapping the loop. Ground Items integration imports highlighted items as supplemental always-loot candidates and records hidden-list, hide-under-value, and show-highlighted-only suppression metadata; hidden/suppressed matches only block pickup when `Respect hidden Ground Items` is enabled and the item is not explicitly allowed by CV Helper. Optional intermediate inventory actions use configurable item-to-action mappings such as `bones -> Bury` and `ashes -> Scatter|Bury`; status reports selected item, slot, matched rule, configured action(s), available item actions, intended/actual action, menu params, result, and failure reason. Missing configured actions are skipped rather than falling back to `Use` or `Drop`, and `Drop` remains blocked for intermediate actions even if it is visible. High Alchemy is currently a safe policy/diagnostic surface: config can enable the policy and set min HA, GE-to-HA delta, max loss, allowlist, and blacklist; status reports candidate items and reasons, but this pass does not invoke the spell until spell/rune availability guards are fully live-verified. Login recovery runs while the live farmer loop is active and reports `macroRunning`, `macroLive`, `recoveryWorkerActive`, `pausedReason`, `willAttemptOnNextRecoveryTick`, preferred/current world readiness, `worldBlockReason`, and `lastClickAttempt` diagnostics including selected login widget, click point, Enter fallback, actual action, and failure reason. Real route pathing, automatic world switching beyond guarded preferred-world validation, emergency teleport, automatic inventory dropping, and external plan composition are follow-up contracts.
- Mining and woodcutting farmers mirror the mob-farmer automation shell through `/automation/mining/status`, `/automation/mining/config`, `/automation/mining/step`, `/automation/mining/start`, `/automation/mining/stop`, `/automation/woodcutting/status`, `/automation/woodcutting/config`, `/automation/woodcutting/step`, `/automation/woodcutting/start`, and `/automation/woodcutting/stop`. Their config payload is `{version:1, skill, settings, schema, presets, status}` with target lists, dry/live default, configurable `scanRadiusTiles`, configurable `maxCandidates`, shared protected items, and report-only inventory policy. The drop-policy status map includes the raw configured `configuredAllowlist` and `configuredProtected` strings so WebHelper can render the exact per-farmer policy tokens without changing drop behavior. They traverse relevant scene tile objects (game, wall, decorative, and ground object layers) within the configured radius, match names or `id:<object id>`, require the correct object action (`Mine` or `Chop down`), choose the nearest reachable object by collision-aware path distance, and expose candidate diagnostics including object type, tile, straight-line distance, path distance, reachable flag, visible flag, canvas bounds/click point, selected object menu action, path search limit, visited count, and rejection reasons. Live mode invokes the menu action matching the object's action index, not a hard-coded first option. The CV Helper overlay can draw latest mining/woodcutting candidate boxes behind the `Show skilling target boxes` config/sidebar toggle. Destructive inventory dropping and respawn-timer plugin integration are still report-only/follow-up.
...
- Mob-farmer combat cadence uses the configurable `attackIntervalTicks` fallback (default 4) as the single global attack scheduler interval. `/automation/mob-farmer/status.combatCadence` reports current/last/estimated-next attack ticks, whether an attack is due, configured interval/source, current target/distance, pending post-action reattack, loot distance, last intermediate action, and the last reattack result. Successful loot, intermediate inventory, and survival inventory actions queue `postActionReattack` (with `reattachAfterPickup` retained as a compatibility alias). While the attack window is cooling down, the existing movement/action continues; when the window opens, the farmer reselects a valid target and reissues Attack even when the loot remains visible. Exact weapon-speed detection is not yet implemented and is reported as unknown.
- Mob-farmer loot travel has separate normal and high-priority path radii. Each candidate reports path distance/reachability, allowed radius/class, estimated travel ticks, estimated missed attacks, and `cadenceDecision`. Normal loot exceeding `normalLootMaxMissedAttacks` while combat is active is skipped to preserve cadence; explicit/high-value priority loot is exempt and reports when it intentionally interrupts combat. Loot pathing may use the existing single safe `Open`/`Close` transition and exposes the same `doorTransition`, `blockedByDoor`, `manualActionRequired`, `manualActionReason`, and `blockingDoor` contract. Multi-door/global navigation remains outside this contract.
- `OSR-6` owns the verifier client site. Browser consumers call CV Helper through `http://127.0.0.1:<port>`, so plugin JSON responses include permissive local-development CORS headers. The verifier helper at `tools/cv-helper-verifier/serve.ps1` serves the static dashboard on `http://127.0.0.1:8765/` and exposes `/api/discover`, which probes `11777` first, then known browser ports, then live local Java listeners. Discovery records current endpoint capabilities and treats an otherwise healthy preferred-port export as stale when it lacks current automation endpoints but another CV Helper listener exposes them.
- WebHelper editing must keep Live State separate from Draft Config. Polling may refresh dashboard/status/raw payloads continuously, but it must not mutate editable fields once a draft exists. The draft can change only through explicit load-current, reset, import, or user edits, and it is applied to RuneLite only through an apply action. If live config changes while a draft is dirty, WebHelper warns instead of overwriting the draft. Raw JSON remains available through Debug/Raw views, while default pages should prefer structured cards, tables, badges, and grouped sections.
- Verifier connection state must distinguish a broken localhost export from a healthy login screen. `LOGIN_SCREEN` or other logged-out states with `/status` responding are success states for transport discovery, even when widget-dependent target surfaces are empty.
...
- `Invoke-RestMethod -Uri http://127.0.0.1:<port>/automation/mob-farmer/config`
- `Invoke-RestMethod -Uri http://127.0.0.1:<port>/automation/mining/config`
- `Invoke-RestMethod -Uri http://127.0.0.1:<port>/automation/woodcutting/config`
- POST invalid mob-farmer config JSON and confirm HTTP `400` includes validation errors and leaves current settings unchanged.
- With auto-refresh enabled, type into a WebHelper config field for at least 30 seconds and confirm polling does not overwrite the dirty draft while live status/raw views continue updating.
...
- Run `powershell -ExecutionPolicy Bypass -File .\tools\cv-helper-verifier\serve.ps1`, open `http://127.0.0.1:8765/`, and verify auto-discovery prefers `11777`, falls back cleanly when needed, distinguishes `LOGIN_SCREEN` from transport failure, and still shows status polling, target tables, warnings, capture paths, and capture buttons.
