...
- Spell action slots default to `AUTO`, which selects/casts spell widgets through RuneLite widget actions rather than physically moving the OS mouse to the spell icon. Targeted spells must use RuneLite `WIDGET_TARGET` selection semantics; self-resolving spells such as teleports may use normal widget operations. `WIDGET` forces this path and fails closed if no widget action exists. `CLICK` forces Java Robot target clicks for debugging or fallback. The OS mouse should only be used for the optional current-target click, return-panel click, and mouse restore unless `CLICK` mode is selected.
- Prayer action slots should invoke the prayer widget through RuneLite widget actions and prefer direct friendly prayer component targets over generic child-widget labels. Targeted spell action slots must wait for selected-widget state before clicking the current mouse target; on timeout, skip the target click rather than walking, and do not click return-panel/restore UI controls because those clicks can cancel or obscure the failed selection state. If click-after is disabled for a targeted spell, `Return tab` must not click a side-panel tab after selecting the spell because that tab click consumes/cancels the selected spell.
- Action timing values should be configurable: panel-open delay, widget-target delay, selected-widget timeout, return-panel delay, and mouse-restore delay. These timings are the first local form of the future external action-sequence executor contract.
