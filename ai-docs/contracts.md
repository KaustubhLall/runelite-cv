...
- Spell action slots default to `AUTO`, which selects/casts spell widgets through RuneLite widget actions rather than physically moving the OS mouse to the spell icon. `WIDGET` forces this path and fails closed if no widget action exists. `CLICK` forces Java Robot target clicks for debugging or fallback. The OS mouse should only be used for the optional current-target click, return-panel click, and mouse restore unless `CLICK` mode is selected.
- Prayer action slots should invoke the prayer widget through RuneLite widget actions and prefer direct friendly prayer component targets over generic child-widget labels. Targeted spell action slots must wait for selected-widget state before clicking the current mouse target; on timeout, skip the target click rather than walking.
