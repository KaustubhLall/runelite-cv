...
- `OSR-10` owns the first mob-farmer automation. Current CV Helper exposes `/automation/mob-farmer/status`, `/automation/mob-farmer/step`, `/automation/mob-farmer/start`, `/automation/mob-farmer/stop`, and `/automation/panic-stop`. The farmer accepts a `target` query parameter with partial name or `id:<npc id>` semantics and a `live=true|false` query parameter. It must fail closed unless logged in, not already interacting, a matching NPC entity has a click point, and no other CV action is running. Dry mode reports the target/click point without clicking and prints it to in-game chat; live mode performs a guarded click. Panic stop stops CV loops, invalidates queued loop steps, interrupts the loop sleeper, and clears the action-in-progress guard. Auto-eat, loot filters, highlighted-drop integration, pathing, and external plan composition are follow-up contracts.
...
- Future farmer loop work should separate the combat loop from the loot loop: prioritize attacking the next mob when combat ends, then pick up allowed/highlighted/valuable drops after the spawn delay. Inventory fullness, HP thresholds, food/potion matching, and exit/pathing are separate guards before longer unattended loops.
