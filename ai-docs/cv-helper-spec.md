...
- Invocation mode is `AUTO`, `WIDGET`, or `CLICK`. `AUTO` prefers RuneLite widget actions for spells/prayers and falls back to physical clicks when no widget action is available. Targeted spells use RuneLite `WIDGET_TARGET` selection semantics; self-resolving spells such as teleports use normal widget operations. `WIDGET` forces widget invocation and fails closed if the target lacks a widget id/action. `CLICK` forces Java Robot physical clicks for debugging or fallback.
...
- Targeted spell actions wait until RuneLite reports a selected widget/spell before clicking the current mouse target. If selection does not happen before timeout, CV Helper skips the target click to avoid accidentally walking and leaves return-panel/mouse-restore clicks alone so the next debug message reflects the true selected-widget state. If click-after is disabled for a targeted spell, `Return tab` is skipped because clicking a side-panel tab would consume/cancel the selected spell.
...
6. Optionally return panel and restore mouse.
