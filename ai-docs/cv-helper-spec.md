...
- Inventory status reports occupied/free slots, protected never-drop list, and safe replacement candidates. Mob Farmer may now use the shared `InventoryDropService` to drop one explicitly allowlisted, low-value slot when inventory is full and a higher-value or required loot candidate is blocked only by capacity. Dry mode reports the exact proposed replacement; live mode invokes the resolved inventory `Drop` menu action and exposes the decision under `status.dropPolicy`.
...
The standalone minimap and embedded farmer grids share one persisted 4-100 tile visual/fetch radius. Transition overlays distinguish direct reachability, a pending transition, multi-transition routes, manual-action-required blockers, and failed steps. A failed step is drawn as a red rotated square on the exact door tile a route could not get through (from a candidate's `failedTransitionIndex` + `blockingDoor`); a successful multi-door route to the selected target is drawn as numbered amber door glyphs along the chain. Common navigational doors/gates (`Door`, `Gate`, `Large door`, `Metal door`, `Wooden door`, `Oak door`, `Sturdy door`) are now in the default door allowlist so they route as transitions out of the box; the denylist always overrides, non-standard doors can be added by object id, and clearing the allowlist returns every door to manual-action-required.
