...
- Loot pickup scans visible scene ground items and reports every candidate in `/automation/mob-farmer/status`. Selection uses explicit always-loot names/ids, Ground Items highlighted/hidden list metadata, minimum GE value, blacklist, ownership policy, inventory capacity, stackability, interaction mode, and score. Normal and high-priority loot use separate local path radii. Candidates expose `pathDistance`, radius class/limit, estimated travel ticks and missed attacks, `cadenceDecision`, `highPriority`, and `priorityReasons`. Normal loot whose travel exceeds the configured missed-attack budget is skipped during active combat; explicit/high-value priority loot may intentionally interrupt cadence. The path check can return the existing one-hop safe door/gate transition for loot, or the explicit manual-action-required diagnostics when it cannot.
...
- Successful live `Take`, intermediate inventory, and survival inventory actions queue a post-action reattack. The next logged-in ticks still run survival first, then use idle cooldown ticks for the already-issued action/movement and reselect/reissue Attack when the configured fallback attack window opens. Loot remaining visible no longer suppresses that attack; subsequent normal flow can resume the loot movement, producing a conservative stutter-step. `/automation/mob-farmer/status.postActionReattack` reports pending state, action/label, queued and attempted ticks, current target, whether reattack was attempted, and a skip reason; `reattachAfterPickup` remains a compatibility alias. `/automation/mob-farmer/status.combatCadence` reports the configured interval and explicitly states that weapon speed is not yet known.
...
Mining and Woodcutting expose a shared target-lifecycle diagnostic shape: `lastCompletedTarget`, `completionReason`, and `lastInvalidationTick`; Mining additionally exposes `rejectedStaleTiles`. Mining XP gains and observed rock identity changes invalidate the active tile before rescan. Woodcutting tree disappearance or loss of `Chop down` invalidates the prior target even when no replacement is currently selectable. The Woodcutting drop allowlist defaults to `logs`.
