...
- Implemented on 2026-06-19: first mob-farmer slice has guarded dry/live step plus dry/live loop controls in the CV Helper panel and `/automation/mob-farmer/*` endpoints. It clicks only when logged in, not already interacting, target has a click point, and no other CV action is running.
