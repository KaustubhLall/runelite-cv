...
- Fixed on 2026-06-19: panel switching for action hotkeys now uses configurable side-panel keybinds for both required panel opens and return-to-previous-panel behavior. Do not restore tabs through widget actions, menu actions, or mouse clicks while a targeted spell/action is selected and unconsumed; the next mouse click must be the intended game target.
