...
- Implemented on 2026-06-20: mob farmer loot and inventory action hardening. Auto-eat and intermediate bones/ashes now invoke inventory widget menu actions directly (`Eat`/`Drink`/`Consume`, then `Bury`/`Scatter`/`Use`) rather than relying on generic clicks. Live loot pickup revalidates the ground item on its scene tile immediately before `Take`; stale or newly rejected loot returns false so `Attack before loot` off cannot trap the loop on an unlootable item. `/automation/mob-farmer/status` now includes `lastActionAttempt` and `recentMenuEntries` so hidden/stacked menu-action failures can be compared against exact RuneLite menu params.
- Current automation follow-up order: real route/path distance; automatic low-value dropping with protected never-drop enforcement; emergency teleport on no-food/low-HP danger; multi-target/profile UX polish; login/reconnect/world recovery; pathing/exit strategy.
